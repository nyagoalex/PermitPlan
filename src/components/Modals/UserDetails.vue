          ...
          <div class=" col-lg-12 col-sm-12 col-12 main-section text-center">
              ...
              <div class="row user-detail">
                  <div class="col-lg-12 col-sm-12 col-12">
                      ...
                      <small><b-icon icon="person-check"></b-icon> {{user.role}}</small>
                      ...
                      <hr>
                      <span class="m-2"><b-icon icon="envelope"></b-icon> <a :href="`mailto:${user.full_name}`">{{user.email}}</a></span>
                      <span class="m-2"><b-icon icon="question-circle"></b-icon> <a :href="`tel:${user.phone}`">{{user.phone}}</a></span>
                  </div>
              </div>
              <div class="row user-actions">
                  <div class="col-lg-12 col-sm-12 col-12">
                    <b-button pill size="sm" :disabled="loading" class="m-2" variant="outline-success" v-if="!user.active" @click="activate"><b-icon icon="check-circle"></b-icon> activate</b-button>
                    <b-button pill size="sm" :disabled="loading" class="m-2" variant="outline-danger" v-else @click="deactivate"><b-icon icon="slash-circle"></b-icon> deactivate</b-button>
                    <b-button pill size="sm" :disabled="loading" class="m-2" variant="outline-warning"><b-icon icon="shield-slash"></b-icon> unblock</b-button>
                    <b-button pill size="sm" :disabled="loading" class="m-2" variant="danger" @click="deleteUser"><b-icon icon="trash"></b-icon> Delete</b-button>
                  </div>
              </div>
          </div>
        </div>
