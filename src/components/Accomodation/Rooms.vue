    ...
    <div class="pt-4">
        <div class="row">
            ...
            <div class="col-md-5 col-12 pt-4">
                <div class='season m-5 ' v-if="!rooms || !rooms.length">
                    <p class='text-muted p-2'>No Room Types Registered Yet.</p>
                </div>
                <div class="season mb-3" v-for="(room, index) in rooms" :key="room.id">
                    <div class="row">
                        ...
                        <div class="col-md-3 text-right">
                            <span style="cursor: pointer;" v-b-tooltip.hover title="Edit Room Type" @click="updateRoom(index)">
                                <b-icon icon="pen"></b-icon>
                            </span> <br>
                            <span style="cursor: pointer;" class="text-danger " v-b-tooltip.hover title="Remove Room Type" @click="removeRoom(index)">
                                ...
                                <b-tbody>
                                    <b-tr>
                                        <b-th v-for="(cost, index) in room.costs" :key="index">
                                            <input type="number" class="form-control" style="min-width:120px;" v-model="cost.amount">
                                        </b-th>
                                        <b-th>
                                            <b-button variant="outline-success" @click="updateCosts(index)">Update</b-button>
                                        </b-th>
                                    </b-tr>
                                </b-tbody>
                            </b-table-simple>
...
export default {
    ...
    methods: {
        addRoom() {
            ...
            this.$http.post('/lodges/' + this.$route.params.id + '/rooms', param)
                .then(response => {
        ...
        updateRoom(index) {
            ...
            this.$swal.fire({
                ...
                if (result.value) {
                    ...
                    this.$http.patch('/lodges/' + this.$route.params.id + '/rooms/' + id, param)
                        .then(response => {
        ...
        updateCosts(index) {
            ...
            this.$http.patch('/rooms/' + room.id + '/costs', param)
                .then(response => {
