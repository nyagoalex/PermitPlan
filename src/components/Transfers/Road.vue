    ...
    >
      <template v-slot:table-caption>
        <b-row>
          ...
          <b-col><b-button  v-b-modal.new-road-transfer class='float-right' size="sm"  variant="success"><b-icon icon="plus"></b-icon> Add Road Transfer</b-button></b-col>
        </b-row>
      </template>
...
export default {
  ...
  methods: {
    ...
    fakeBookings () {
      const s = []
      for (let i = 1; i < 20; i++) {
        const v = { no: i, name: faker.name.lastName(), starting_point: faker.address.state(), destination: faker.address.state(), vehicle: faker.name.lastName(), cost_per_person: faker.random.number(), distance: faker.random.number(), est_time: '16:05:00 Hrs' }
        s.push(v)
      }
      return s
    }
