    ...
    >
      <template v-slot:table-caption>
        <b-row>
          ...
          <b-col><b-button v-b-modal.new-lodge class='float-right' size="sm"  variant="success"><b-icon icon="plus"></b-icon> Add Lodges / Hotels</b-button></b-col>
        </b-row>
      </template>
      <template v-slot:cell(#)="row"> {{ row.index + 1 }} </template>
      <template v-slot:cell(hotel_star_rating)="row">
        <span><star-rating :rating="row.item.star_rating" :star-size="20" :read-only="true" :increment="0.01" :show-rating="false"></star-rating></span>
      </template>
    </b-table>
    <div>
...
export default {
  data () {
    return {
      ...
      fields: [
        ...
        { key: 'contracted_rating', sortable: true }
      ],
      lodges: [],
      mode: '',
  ...
  methods: {
    getLodges (page = 1) {
      ...
      filters.page = page
      this.$http.get('/lodges', { params: filters }).then(lodges => {
    ...
    onRowClicked (item, index, event) {
      this.$router.push({ name: 'AccommodationDetails', params: { id: item.id } })
    }
  },
  mounted () {
    this.getLodges()
